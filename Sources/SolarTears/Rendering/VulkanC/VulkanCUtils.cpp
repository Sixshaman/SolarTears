...
VulkanCBindings::VulkanUtils::VkException::VkException(VkResult res, const std::wstring& funcionName, const std::wstring& filename, int lineNumber): ErrorCode(res), FunctionName(funcionName), Filename(filename), LineNumber(lineNumber)
{
}

std::wstring VulkanCBindings::VulkanUtils::VkException::ToString() const
{
    return FunctionName + L" failed in " + Filename + L"; line " + std::to_wstring(LineNumber) + L"; error code: " + std::to_wstring(ErrorCode);
}

VkDeviceSize VulkanCBindings::VulkanUtils::AlignMemory(VkDeviceSize value, VkDeviceSize alignment)
{
    return value + (alignment - value % alignment) % alignment;
}

void VulkanCBindings::VulkanUtils::LoadShaderModuleFromFile(const std::wstring& filename, std::vector<uint32_t>& dataBlob, LoggerQueue* logger)
{
...
VkBool32 VulkanCBindings::VulkanUtils::DebugReportCCallback(VkDebugReportFlagsEXT flags, VkDebugReportObjectTypeEXT objectType, uint64_t object, size_t location, int32_t messageCode, const char* pLayerPrefix, const char* pMessage, void* pUserData)
{
