...
{
	constexpr int currentYear = ((__DATE__[7] - '0') * 1000 + (__DATE__[8] - '0') * 100 + (__DATE__[9] - '0') * 10 + (__DATE__[10] - '0') * 1);
	return CurrentYear() == YearFromIso8601(date);
}

inline constexpr bool IsMonthInPresent(const std::string_view date)
{
	constexpr int currentMonth = ((__DATE__[0] == 'J') ? ((__DATE__[1] == 'a') ? 1 : ((__DATE__[2] == 'n') ? 6 : 7)) : ((__DATE__[0] == 'F') ? 2 : ((__DATE__[0] == 'M') ? ((__DATE__[2] == 'r') ? 3 : 5) : ((__DATE__[0] == 'A') ? ((__DATE__[1] == 'p') ? 4 : 8) : ((__DATE__[0] == 'S') ? 9 : ((__DATE__[0] == 'O') ? 10 : ((__DATE__[0] == 'N') ? 11 : 12)))))));
	return CurrentMonth() == MonthFromIso8601(date);
}

inline constexpr bool IsDayInPresent(const std::string_view date)
{
	constexpr int currentDay = ((__DATE__[4] - '0') * 10 + (__DATE__[5] - '0') * 1);
	return CurrentDay() == DayFromIso8601(date);
}
